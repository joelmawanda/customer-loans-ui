              ...
              tag.values.forEach((outcome) => {
                switch (outcome) {
                  case "SUCCESS":
                    ...
                    setNumPositiveRequests(
                      (prevNumPositiveRequests) => prevNumPositiveRequests + 1
                    );
                    break;
                  case "CLIENT_ERROR":
                    // setNumNegativeRequests(numNegativeRequests + 1);
                    setNumNegativeRequests(
                      (prevNumNegativeRequests) => prevNumNegativeRequests + 1
                    );
                    break;
                  default:
                    break;
            ...
            case "error":
              tag.values.forEach((value) => {
                if (value !== "none") {
                  ...
                  setNumFailedValidations(
                    (prevNumFailedValidations) => prevNumFailedValidations + 1
                  );
                }
              });
              break;
